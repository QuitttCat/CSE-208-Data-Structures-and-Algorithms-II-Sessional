...
int main() {
    int n = 42; // Number of items
    int W = 715; // Maximum weight capacity

    std::random_device rd;
    std::mt19937 gen(rd());

    // Value range and weight range
    std::uniform_int_distribution<int> val_dist(1401,1402);
    std::uniform_int_distribution<int> weight_dist(71,72);

    std::ofstream outFile("test_case4.txt");
    if (!outFile.is_open()) {
